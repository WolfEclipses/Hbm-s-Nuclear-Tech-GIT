...
import com.hbm.inventory.container.ContainerMachineRouter;
...
public class TileEntityMachineRouter extends TileEntityMachineBase implements IEnergyReceiverMK2, IFluidStandardTransceiverMK2, IConditionalInvAccess, IGUIProvider, IUpgradeInfoProvider, IFluidCopiable{
	...
	public TileEntityMachineRouter() {
		super(8);
	...
	public String getName() {
		return "container.machineRouter";
	}
		...
		if(!worldObj.isRemote) {
			...
			if(this.power >= 100) {
				...
				if(!isOn) {
					...
					this.worldObj.func_147451_t(xCoord, yCoord, zCoord);
				}
			...
			RouterRecipe recipe = RouterRecipes.getRecipe(slots[0], slots[1], slots[2]);
	...
	public boolean canProcess(RouterRecipe recipe) {
	...
	public void consumeItems(RouterRecipe recipe) {
	...
	public DirPos[] getConPos() {
		...
		return new DirPos[] {
			new DirPos(xCoord - dir.offsetX * 3, yCoord, zCoord - dir.offsetZ * 3, dir.getOpposite()),
			new DirPos(xCoord - dir.offsetX * 3 + rot.offsetX * 3, yCoord, zCoord - dir.offsetZ * 3 + rot.offsetZ * 3, dir.getOpposite()),
			new DirPos(xCoord - rot.offsetX, yCoord, zCoord - rot.offsetZ, rot.getOpposite()),
			new DirPos(xCoord - dir.offsetX * 2 - rot.offsetX, yCoord, zCoord - dir.offsetZ * 2 - rot.offsetZ, dir.getOpposite()),
			new DirPos(xCoord + rot.offsetX * 4, yCoord, zCoord + rot.offsetZ * 4, rot),
	...
	public Container provideContainer(int ID, EntityPlayer player, World world, int x, int y, int z) {
		return new ContainerMachineRouter(player.inventory, this);
	}

	@Override
	@SideOnly(Side.CLIENT)
	public Object provideGUI(int ID, EntityPlayer player, World world, int x, int y, int z) {
		return new GUIMachineRouter(player.inventory, this);
	}
	...
	public AxisAlignedBB getRenderBoundingBox() {
		...
		if(bb == null) bb = AxisAlignedBB.getBoundingBox(xCoord - 1, yCoord, zCoord - 1, xCoord + 4, yCoord + 3, zCoord + 3);
	...
	public void provideInfo(UpgradeType type, int level, List<String> info, boolean extendedInfo) {
		info.add(IUpgradeInfoProvider.getStandardLabel(ModBlocks.machine_router));
